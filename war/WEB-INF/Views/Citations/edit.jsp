...
<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <% 
    ...
    String dateString = formato.format(citation.getDate());
	  ...
	  <div class="row content">
	    <div class="col-sm-2 sidenav">
	       <div class="navbar-nav mr-auto  text-left">
				    ...
				    <a class="nav-item nav-link" href="/citations">List Citations</a>
			</div>
	    </div>
	    <div class="col-sm-10 text-left"> 
	    <br>
	      <h3 class="text-primary">Edit Citation</h3>
		  <form method="post"  action="/citations/edit">
		   <div class="form-group">
			   <label>Id:</label>
			   <input type="text"  class="form-control" readonly name="id" value= "<%=citation.getId()%>">
		  </div>
		   <div class="form-group">
			    <label>Name:</label>
			    <input type="text" class="form-control" name="name" value= "<%=citation.getName()%>" placeholder="Name" required autofocus>
		   </div>
		   <div class="form-group">
			    <label>Type:</label>
			    
			    <%if(citation.getType().equals("Homework")){ %>
			    <select class="form-control" name="type"  required>
				 ...
				 <%}    
			    else if(citation.getType().equals("Event")){ %>
			    <select class="form-control" name="type"  required>
				 ...
				 <%}  
			    else if(citation.getType().equals("Exam")){ %>
			    <select class="form-control" name="type"  required>
				      <option value="Homework" >Homework</option>
				      <option value="Event" >Event</option>
		   ...
		   <div class="form-group">
			    <label>Date:</label>
			    <input type="date" class="form-control" name="date" placeholder="Date" value="<%=dateString%>" max="2050-05-24" required>
		   </div>
		   <div class="form-group">
			    <label >Description:</label>
			    <textarea class="form-control" name="description" rows="3" placeholder="Description" required><%=citation.getDescription()%></textarea>
		   </div>
