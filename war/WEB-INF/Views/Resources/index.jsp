	  ...
	  <div class="row content">
	    <div class="col-sm-2 sidenav">
	       <div class="navbar-nav mr-auto  text-left">
				    ...
				    <a class="nav-item nav-link" href="/resources/add">New Resource</a>
			</div>
	    </div>
	    ...
	    <br>
		  ...
		  <table class="table table-bordered">
		    ...
		    <tbody>
              ...
              <% 
				List<Resource> array = (List<Resource>)request.getAttribute("resources");
			
				if(array.size() > 0) {
					
					for(Resource rol : array){
		      %>
		      <tr>
		        <td><%=rol.getId() %></td>
		        <td><%=rol.getUrl()%></td>
		        <td><%=rol.isStatus() %></td>
		        <td><%=rol.getCreated() %></td>
		        <td>
		        <a href="/resources/view?id=<%=rol.getId()%>">View</a>  
		        <a href="/resources/edit?id=<%=rol.getId()%>">Edit</a>  
		        <a href="/resources/delete?id=<%=rol.getId()%>">Delete</a>
		        </td>
		      </tr>
		      	<% } } %>
		    </tbody>
